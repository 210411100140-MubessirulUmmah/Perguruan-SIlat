...
$query = "SELECT * FROM rekomendasi_lomba WHERE id_rekomendasi = $id_rekomendasi";
$result = mysqli_query($koneksi, $query);
if ($result) {
    $hasil = mysqli_fetch_assoc($result);
} else {
    echo "Gagal mendapatkan informasi Lomba.";
}
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Super Admin | Edit Rekomendasi Lomba</title>
        ...
        <div class="content-wrapper" style="background-color: #18191A;">
            ...
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 style="color: #fff;margin-top:70px;"><b>Edit rekomendasi lomba</b></h1>
                        </div>
                    </div>
                </div><!-- /.container-fluid -->
            </section>
                        ...
                        <div class="col-12">
                            ...
                            <div class="card">
                                <div class="top-section" style="background-color: #000;color: #fff;">
                                    ...
                                    <h1 style="color: #fff; background-color:#000; text-align:center">Form Edit rekomendasi lomba</h1>
                                </div>
                                <div class="card-body">
                                    <form action="proses_editlomba.php" method="post" enctype="multipart/form-data">
                                        <input type="hidden" name="id_rekomendasi" value="<?php echo $hasil['id_rekomendasi']; ?>"> <!-- Anda perlu menyimpan ID berita untuk proses edit -->
                                        <label for="judul">Judul:</label>
                                        <input type="text" id="judul" name="judul" value="<?php echo $hasil['nama_perlombaan']; ?>" required><br><br>

                                        <label for="deskripsi">Deskripsi:</label><br>
                                        <textarea id="deskripsi" name="deskripsi" rows="4" cols="50" required><?php echo $hasil['deskripsi_lomba']; ?></textarea><br><br>
                                        <label for="tempat">Tempat:</label><br>
                                        <textarea id="tempat" name="tempat" rows="4" cols="50" required><?php echo $hasil['tempat_lomba']; ?></textarea><br><br>
                                        <label for="tingkat">Tingkat Perlombaan:</label>
                                        <select id="tingkat" name="tingkat" required>
                                            <option value="" disabled selected hidden>Pilih Kategori</option>
                                            <option value="nasional" <?php if ($hasil['tingkat_perlombaan'] == "nasional") echo "selected"; ?>>nasional</option>
                                            <option value="internasional" <?php if ($hasil['tingkat_perlombaan'] == "internasional") echo "selected"; ?>>internasional</option>
                                        </select><br><br>

                                        <label for="jurusan">Jurusan:</label>
                                        <select id="jurusan" name="jurusan" required>
                                            <option value="" disabled selected hidden>Pilih jurusan</option>
                                            <option value="seni" <?php if ($hasil['jenis_perlombaan'] == "seni") echo "selected"; ?>>Seni</option>
                                            <option value="tanding" <?php if ($hasil['jenis_perlombaan'] == "tanding") echo "selected"; ?>>Tanding</option>
                                        </select><br><br>
                                    ...
                                    <!-- Tombol Hapus -->
                                    <form action="proses_editlomba.php" method="post">
                                        <input type="hidden" name="id_lomba" value="<?php echo $hasil['id_rekomendasi']; ?>">
                                    </form>
                                </div>
                            </div>
                            <!-- /.card -->
